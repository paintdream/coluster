...
struct float3 {
	...
	float3(float xx = 0.0f, float yy = 0.0f, float zz = 0.0f) noexcept : x(xx), y(yy), z(zz) {}
	float x, y, z;
};
...
static box build_box(const float3& first, const float3& second) noexcept {
	box box;
	box.first.x = std::min(first.x, second.x);
	box.first.y = std::min(first.y, second.y);
	box.first.z = std::min(first.z, second.z);
	box.second.x = std::max(first.x, second.x);
	box.second.y = std::max(first.y, second.y);
	box.second.z = std::max(first.z, second.z);

	return box;
}

class sample_tree : public tree_t<box> {
public:
	using base = tree_t<box>;
	sample_tree() noexcept : tree_t<box>(box(float3(0, 0, 0), float3(0, 0, 0)), 0) {}
	sample_tree(const box& b, uint8_t k) noexcept : tree_t<box>(b, k) {}
};
...
size_t linear_search(sample_tree* root, std::vector<sample_tree>& nodes, const box& box) noexcept {
	size_t count = 0;
...
int main(void) {
	...
	for (size_t k = 0; k < nodes.size() / 8; k++) {
		...
		sample_tree* new_root = (sample_tree*)to_detach->detach(random_selector);
		if (new_root != nullptr) {
			root = new_root;
			if (k & 1) {
